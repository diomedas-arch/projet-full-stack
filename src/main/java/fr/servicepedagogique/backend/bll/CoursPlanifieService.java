...
public class CoursPlanifieService {
    ...
    private static final String STATUT_PAR_DEFAUT = "PLANIFIE";
    ...
    @Transactional(readOnly = true)
    public List<CoursPlanifieResponse> findByPromotion(Integer idPromotion) {
    ...
    @Transactional(readOnly = true)
    public List<CoursPlanifieResponse> findByFormateur(Integer idFormateur) {
    ...
    public CoursPlanifieResponse creer(CreerCoursPlanifieRequest request) {
        ...
        CoursPlanifie coursPlanifie = new CoursPlanifie(
                promotion, cursusCours, formateur,
                request.dateDebut(), request.dateFin(), request.salle(), statut
        );
    ...
    public CoursPlanifieResponse modifier(Integer idCoursPlanifie, ModifierCoursPlanifieRequest request) {
        ...
        if (request.salle() != null) {
            coursPlanifie.setSalle(request.salle());
        }

        if (request.statut() != null) {
            coursPlanifie.setStatut(request.statut());
        }
    ...
    private void verifierCursusCoursAppartientAuCursusDeLaPromotion(Promotion promotion, CursusCours cursusCours) {
        if (!cursusCours.getCursus().getIdCursus().equals(promotion.getCursus().getIdCursus())) {
            throw new IllegalArgumentException(
                    "Le cours planifié doit appartenir au même cursus que la promotion.");
        }
    }

    private void verifierCoupleDisponible(Promotion promotion, CursusCours cursusCours, CoursPlanifie coursPlanifieActuel) {
